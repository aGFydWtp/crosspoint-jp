...
このフォークは独自に x.y.z を進めない。タグは常に「取り込み済みの CrossPointJP ベース版 + `-forked.<N>`」
（例: `v0.1.16-forked.1`）で、`release-dispatch.yml` が祖先の直近リリースタグから自動採番する。

| 入力 | 説明 |
|------|------|
| `dry_run` | 算出結果をサマリに出すだけで、タグは打たない |

タグを push したあと `release-dispatch.yml` が `release.yml` を起動し、ビルドと GitHub Release の作成、
Deploy Flasher（Web Flasher のマニフェスト更新）まで自動で進む。`-forked` タグはハイフンを含むが
prerelease 扱いにはならない（release.yml 側に例外がある）。
