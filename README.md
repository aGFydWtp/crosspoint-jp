...
- **青空文庫TXT→EPUB変換** — WebUIのファイル転送時に、青空文庫形式のTXTをEPUBに変換して転送可能（オプトイン、Shift_JIS自動判別、変換失敗時は元のTXTのまま転送）

### マイXTC（html2xtc連携）

- ホーム画面の「マイXTC」から、html2xtcサーバー上の自分の書庫（OPDSカタログ）をブラウズ・ダウンロード
- 現状は開発者向けマイルストーン: SDカードの `/.crosspoint/html2xtc.json` にペアリング情報（`deviceId` / `deviceToken`）を手動配置して利用（QRコードによるペアリングは今後対応予定）

### Sleep画面

- お気に入りの画像をスリープ画面に設定（WebUIからアップロード）
- 画像フィルターを選択可能（デフォルト: 白黒）
- 当月カレンダーをオーバーレイ表示（配置位置は設定で変更可能、初回のみWiFi接続による時刻同期が必要）

### 操作

- **傾きページ送り** — 端末を左右に傾けてページ送り（IMUセンサー搭載機のみ、設定で有効化）
- **手動画面リフレッシュ** — 電源ボタン短押しで画面の残像をクリア

### その他

- **読書進捗アイコン** — ファイル一覧・最近読んだ本リストで、未読/読書中/読了をアイコンで識別可能（95%以上読了で自動判定）
- **WiFi経由の本体アップデート（OTA）** — 設定メニューから本フォークの最新安定版を確認してそのまま更新可能（v0.1.12でダウンロードの安定性・失敗時の診断表示を大幅改善）
...
- WiFi接続時のNTP時刻自動同期
- 各種バグ修正・安定性改善（縦書きでの超長段落EPUBのクラッシュ修正、スリープ壁紙が数秒後に淡くなる不具合の修正など）

## インストール

### ⚠️ USBロックされたデバイスについて

近年出荷されている特定ロケール版のXteinkデバイス（海外向けモデル、AliExpressで販売されている個体の多く）は、**工場出荷時にUSB経由のフラッシュが無効化**されています。該当するデバイスでは、以下の Web Flasher・esptool 等によるUSB経由の書き込みは**一切利用できません**。

該当する場合は、本家CrossPointが提供する **[OTA Unlocker](https://crosspointreader.com/unlock)** を使用してください。これはPCを一時的にWi-Fiホットスポット化し、純正ファームウェアのアップデート確認に割り込んでCrossPointを配信するツールです。

**USBロック機での本フォーク導入手順:**

1. [OTA Unlocker](https://crosspointreader.com/unlock) の指示に従い、本家CrossPointをデバイスに導入する
2. SDカード直下に本フォークの `firmware.bin` を配置し、SDカードファーム更新機能で書き込む（または本フォーク導入後はOTA更新も利用可能）

> **⚠️ 文鎮化リスク:** USBロック機はOTA・SDカード経由でしかファームウェアを書き換えられません。OTA更新やSDカード更新の手段を持たないファームウェアを書き込むと、**復旧不能になる恐れがあります**。本フォークはSDカードファーム更新・OTA更新の両方をサポートしています。

### Web Flasher（推奨・USBロックされていないデバイスのみ）
...
USB経由での書き込みができない場合、以下の2つのケースが考えられます。

### ケース1: USBロックされたデバイス

前述の「USBロックされたデバイスについて」を参照してください。この場合はハードウェア側の仕様であり、USB経由での書き込みはできません。[OTA Unlocker](https://crosspointreader.com/unlock) を使用してください。

### ケース2: v0.1.9 未満の既知バグ

**本フォークの v0.1.9 未満のバージョン** をお使いの場合、既知の不具合により以下の症状で更新ができない可能性があります：

- PC (macOS / Windows / Linux) で USB接続してもデバイスが認識されず、`pio` や esptool でフラッシュできない
- Web Flasher でもシリアルポート選択画面にデバイスが出てこない

**原因**: X3の充電状態判定が USB Serial の初期化条件になっていたため、バッテリー100%充電時に USB CDC が構成されないバグ（v0.1.9で修正済み）。詳細は [PR #69](https://github.com/zrn-ns/crosspoint-jp/pull/69) を参照。
...
書き込み完了後は SDカードファーム更新機能・OTA更新が使えるようになるので、以降のアップデートは容易です。
...
> **注:** TXT/Markdown形式はSDカードフォント使用時のテキストレイアウト計算にパフォーマンス上の問題があるため、デバイス上で直接開くことはできません。ただし青空文庫形式のTXTは、WebUIのファイル転送時にEPUBへ変換して取り込むことができます（前述「青空文庫」参照）。
